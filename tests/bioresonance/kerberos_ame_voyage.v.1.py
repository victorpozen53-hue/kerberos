...
# ║  • 3 APIs de géocodage (Nominatim, GeoNames, Photon) + Cache       ║
# ║  • MapQuest optionnel (clé requise, non obligatoire)               ║
# ║  • Calcul des vies antérieures (voyage de l'âme)                   ║
# ║  • Carte interactive des incarnations passées                      ║
# ║  • Détection de patterns karmiques géographiques                   ║
# ║  • Options : Origine cosmique, Humour sacré, Mode lignée           ║
# ╚══════════════════════════════════════════════════════════════════════╝
...
class GeoCodageAvance:
    ...
    def _geocode_nominatim(self, adresse):
        ...
        # CORRIGÉ : suppression des espaces après q=
    ...
    def _geocode_mapquest(self, adresse):
        if not self.mapquest_key:
            return None
        query = urllib.parse.quote(adresse)
        # CORRIGÉ : suppression des espaces après key=
...
class PanelAmeFull:
    ...
    def calculer_voyage_ame(self):
        try:
            ...
            # TRACEBACK DÉTAILLÉ POUR DIAGNOSTIC
            error_detail = traceback.format_exc()
            self.log_msg("=== ERREUR DÉTAILLÉE ===\n" + error_detail)
            messagebox.showerror("❌ Erreur Cerveau", f"{type(e).__name__}: {e}\n(Voir journal)")
