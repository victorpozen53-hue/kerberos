...
# -*- coding: utf-8 -*-
"""
🛰️ KERBEROS LIDAR GUI — v3.3 WAR MODE (machine de guerre géométrique)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Author: Victor Pozen | GPLv3
Blindage v3.2 conservé (imports gardés, callbacks try/except, thread isolé,
console qui reste ouverte, chaînes une ligne).
NOUVEAU : classifieur de formes -> matériel :
- TRIANGLE  = chasseur à voilure delta   (orange)
- RECTANGLE = camion / voiture           (jaune)
- CARRE     = batterie / canon           (magenta)
- CROIX     = avion commercial           (blanc)
- ROND      = dôme / radar               (rouge)
- LIGNE     = piste / route              (rouge sombre)
"""
import sys
import threading
import math
...
# --- WAR MODE : forme -> matériel (label, couleur BGR) ---
WAR = {
    "DELTA": ("CHASSEUR DELTA", (0, 165, 255)),
    "CAMION": ("CAMION/VOITURE", (0, 255, 255)),
    "CARRE": ("BATTERIE/CANON", (255, 0, 255)),
    "CROIX": ("AVION COMMERCIAL", (255, 255, 255)),
    "DOME": ("DOME/RADAR", (80, 80, 255)),
    "LINEAIRE": ("PISTE/ROUTE", (80, 80, 255)),
}


def classify_shape(cnt):
    """La grammaire de guerre : la géométrie dit le matériel."""
    ...
    solidity = area / harea if harea else 0
    x, y, w, h = cv2.boundingRect(cnt)
    ar = float(w) / max(1, h)
    if v == 3 and solidity > 0.8:
        return "DELTA"
    if v == 4:
        return "CARRE" if 0.85 <= ar <= 1.15 else "CAMION"
    if v >= 8 and solidity < 0.6:
        return "CROIX"
    return None
...
class LidarCore:
    ...
    def _is_ui(hsv, x, y, w, h):
        ...
        if m_s < 25 and m_v > 140:
            return True
        if 15 <= m_h <= 35 and m_s > 120 and m_v > 180:
            return True
        if (m_h <= 10 or m_h >= 170) and m_s > 120 and m_v > 150:
            return True
        return False
    ...
    def process(self, frame):
        ...
        # WAR MODE : chaque contour dans la fenêtre de taille est classifié
        echoes = []
        ...
        for kind, (x, y, w, h) in self.confirmed:
            label, color = WAR.get(kind, (kind, (80, 80, 255)))
        ...
        cv2.line(frame, (cx, cy),
                 ...
                 (0, 255, 204), 2)
        cv2.putText(frame, f"points:{n} cibles:{len(self.confirmed)}",
                    (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 204), 1)
        cv2.putText(frame, "WAR: jaune=camion orange=delta blanc=croix magenta=carre rouge=dome",
                    (10, self.h if hasattr(self, 'h') else H - 12),
...
class LidarApp:
    ...
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🛰️ KERBEROS LIDAR v3.3 WAR MODE")
        ...
        self._log("✅ WAR MODE armé — la géométrie dit le matériel")
        self.root.mainloop()

    def _build(self):
        tk.Label(self.root, text="🛰️ KERBEROS LIDAR v3.3 — WAR MODE", bg=self.BG_MEDIUM,
        ...
        tk.Label(sf, text="☁️ Points:", bg=self.BG_DARK, fg=self.FG_CYAN).pack(side=tk.LEFT)
        ...
        tk.Label(sf, text="⭕ Circularité:", bg=self.BG_DARK, fg=self.FG_CYAN).pack(side=tk.LEFT, padx=(15, 0))
        ...
        tk.Checkbutton(of, text="📷 SMART JPEG (cibles seulement)", variable=self.smart_var,
                       bg=self.BG_DARK, fg=self.FG_CYAN, selectcolor=self.BG_MEDIUM,
                       activebackground=self.BG_DARK,
                       activeforeground=self.FG_CYAN).pack(side=tk.LEFT, padx=20)

        self.preview = tk.Label(self.root, bg=self.BG_DARK,
                                text="⏳ Choisis un dossier d'images…",
    ...
    def _status(self, fi, tot, e, name):
        ...
        if e:
            self._log(f"🎯 image {fi}: {e} cible(s) confirmée(s)")
    ...
    def _start(self):
        try:
            ...
            if src.is_file() and src.suffix.lower() in VIDEO_SUFFIXES:
                messagebox.showerror("Erreur", "Le LiDAR ne lit QUE les images. Convertis la vidéo avec le recorder.")
                return
            self.stop_flag = False
        ...
        try:
            ...
            for p in files:
                ...
                if first is not None:
                    break
            if first is None:
                self.root.after(0, lambda: messagebox.showerror("Erreur", "Aucune image lisible"))
                return
            h0, w0 = first.shape[:2]
            ...
            self.root.after(0, lambda: self._log(f"▶️ Scan WAR de {len(files)} image(s)"))
                ...
                if smart:
                    if self.core.confirmed:
                        ...
                        if fi not in saved:
                            ...
                            cv2.imwrite(str(jpg_dir / f"cible_{fi:04d}_{k0}.jpg"), frame,
                                        [int(cv2.IMWRITE_JPEG_QUALITY), JPEG_QUALITY])
                            saved.add(fi)
                            jpg_count += 1
                    else:
            ...
            writer.release()
            self.root.after(0, lambda: self._log(f"✅ Terminé: {out} | {jpg_count} JPEG"))
            self.root.after(0, lambda: self.status_label.config(text="✅ Scan terminé — WAR MODE veille"))
        except Exception as e:
